...
$app->post('/encrypt', function(Request $request) use($app, $pub_key_file, $key_name) {
    $key_text = file_get_contents($pub_key_file);
    $string = $request->get('string');
    openssl_public_encrypt($string, $crypt_text, $key_text);
    return $app->json(['encrypted' => base64_encode($crypt_text), 'plain_text' => $string, 'key_name' => $key_name]); 
});

$app->post('/decrypt', function(Request $request) use($app, $priv_key_file, $key_name) { 
    ...
    return $app->json(['plain_text' => $decrypted, 'encrypted' => $string, 'key_name' => $key_name]);
});
